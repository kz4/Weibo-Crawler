...
class launcher():
    ...
    def get_prelogin_args(self):
        ...
        try:
            ...
            print('prelogin response: ' + json_data)
            return data
    ...
    def get_encrypted_name(self):
        ...
        encrypted_name = username_encrypted.decode('utf-8')
    ...
    def login(self):
        ...
        try:
            ...
            response = urllib.request.urlopen(request)
            htmlBeforeDecoding = response.read()
            print('********************** html start **********************')
            print(htmlBeforeDecoding)
            print('********************** html end **********************')
            html = htmlBeforeDecoding.decode('GBK')
        except urllib.error as e:
            print(e.code)
        ...
        try:
            ...
            print('redirect_url: ' + login_url)
            request = urllib.request.Request(login_url)
            response = urllib.request.urlopen(request)
            final = response.read().decode('utf-8')
            # writeToDisk("test_output.html", '********************** final **********************')
            writeToDisk("test_output.html", final)
            # writeToDisk("test_output.html", '********************** final **********************')
            print("Login success!")
